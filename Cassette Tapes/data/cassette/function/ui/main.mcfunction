...
execute as @a[scores={ui.id=1..}] if score @s ui.id = .in ui.id at @s anchored eyes positioned ^ ^ ^ as @e[type=chest_minecart,tag=ui] if score @s ui.id = .in ui.id run tp @s ~ ~-2.31 ~
...
execute if score #temp ui matches 1 as @e[type=chest_minecart] if score @s ui.id = .in ui.id run function cassette:ui/refresh
